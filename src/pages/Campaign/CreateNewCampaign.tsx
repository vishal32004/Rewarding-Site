...
const CreateNewCampaign = () => {
  ...
  const {
    ...
    queryFn: () => fetchEventByParentId(forWho),
    enabled: !!forWho,
    staleTime: 5 * 60 * 1000,
  });
  const {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="mb-8 text-2xl">Create New Campaign</h2>
          ...
          <>
              ...
              {events?.data?.some((event) => event.children?.length > 0) ? (
                events.data.map((parentEvent) => (
                  <CustomFormField
                    ...
                    label={parentEvent.title}
                    accordionValue={parentEvent.title}
                    radioOptions={parentEvent.children.map((childEvent) => ({
                ...
                <CustomFormField
                  ...
                  radioOptions={events?.data?.map((event) => ({
                    label: event.title,
                    value: event.id,
                  }))}
                />
            ...
            <div className="mt-5">
              <CustomFormField
                ...
                fieldType={FormFieldType.INPUT}
                label="Other Event"
                placeholder="Enter other event name"
              />
            </div>
          </>
        )}
      ...
      >
        <h2 className="my-4 text-center text-3xl">Select Distribution Type</h2>
      ...
      {distributionType !== "bulk_order" && (
        <>
          ...
          <WizardStep step={8} validator={() => validateStep(stepFields[8])}>
            <h2 className="my-4 text-center text-3xl">Select Email Template</h2>
            <div className="space-y-6">
              {/* <CustomFormField
                  control={form.control}
                  name="emailTemplate"
